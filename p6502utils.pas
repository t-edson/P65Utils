...
begin
  ...
  //Codifica parámetros
  case addMode of
  aImplied_, aAcumulat:;  //No tiene parámetros
  aImmediat: begin
...
function TP6502.codInsert(iflash0, nInsert, nWords: integer): boolean;
...
begin
  ...
  if iRam+nInsert+nWords-1> MaxFlash then begin
    //Overflow on address
    exit(false);
  end;
...
//procedure TP6502.BTFSC_sw_BTFSS(iflash0: integer);
...
begin
  ...
  %00000000: ram[parIns              ].value := value;
  %00100000: ram[parIns+PICBANKSIZE  ].value := value;
  %01000000: ram[parIns+PICBANKSIZE*2].value := value;
  %01100000: ram[parIns+PICBANKSIZE*3].value := value;
  end;
end;
...
begin
  ...
  case STATUS and %01100000 of
  %00000000: Result := ram[parIns                 ].value;
  %00100000: Result := ram[parIns+ PICBANKSIZE  ].value;
  %01000000: Result := ram[parIns+ PICBANKSIZE*2].value;
  %01100000: Result := ram[parIns+ PICBANKSIZE*3].value;
  end;
end;
...
function TP6502.BankToAbsRAM(const offset, bank: byte): word;
{Convierte una dirección y banco a una dirección absoluta}
begin
  Result := bank * PICBANKSIZE + offset;
end;
procedure TP6502.AbsToBankRAM(const AbsAddr: word; var offset, bank: byte);
...
const
  ...
  begin
    ...
    while (iHex<PICMAXFLASH) and not ram[iHex].used  do begin
      inc(iHex);
    end;
    ...
    while (iHex<PICMAXFLASH) and (cont<MAX_INS_HEX) and ram[iHex].used do begin
      inc(iHex);
      inc(cont);
    end;
    if iHex>=PICMAXFLASH then begin
...
begin
  ...
  PIC16InstName[i_BRK].AddAddressMode(aImplied_,$00,1,7,'');
  ...
  PIC16InstName[i_CLC].AddAddressMode(aImplied_,$18,1,2,'');
  PIC16InstName[i_CLD].name := 'CLD';  //Clear Decimal Mode
  PIC16InstName[i_CLD].AddAddressMode(aImplied_,$D8,1,2,'');
  PIC16InstName[i_CLI].name := 'CLI';  //Clear Interrupt Disable Bit
  PIC16InstName[i_CLI].AddAddressMode(aImplied_,$58,1,2,'');
  PIC16InstName[i_CLV].name := 'CLV';  //Clear Overflow Flag
  PIC16InstName[i_CLV].AddAddressMode(aImplied_,$B8,1,2,'');
  ...
  PIC16InstName[i_DEX].AddAddressMode(aImplied_,$CA,1,2,'');
  PIC16InstName[i_DEY].name := 'DEY';  //Decrement Index Y by One
  PIC16InstName[i_DEY].AddAddressMode(aImplied_,$88,1,2,'');
  ...
  PIC16InstName[i_INX].AddAddressMode(aImplied_,$E8,1,2,'');
  PIC16InstName[i_INY].name := 'INY';  //Increment Index Y by One
  PIC16InstName[i_INY].AddAddressMode(aImplied_,$C8,1,2,'');
  ...
  PIC16InstName[i_NOP].AddAddressMode(aImplied_,$EA,1,2,'');
  ...
  PIC16InstName[i_PHA].AddAddressMode(aImplied_,$48,1,3,'');
  PIC16InstName[i_PHP].name := 'PHP';  //Push Processor Status on Stack
  PIC16InstName[i_PHP].AddAddressMode(aImplied_,$08,1,3,'');
  PIC16InstName[i_PLA].name := 'PLA';  //Pull Accumulator from Stack
  PIC16InstName[i_PLA].AddAddressMode(aImplied_,$68,1,4,'');
  PIC16InstName[i_PLP].name := 'PLP';  //Pull Processor Status fromStack
  PIC16InstName[i_PLP].AddAddressMode(aImplied_,$28,1,4,'');
  ...
  PIC16InstName[i_RTI].AddAddressMode(aImplied_,$40,1,6,'');
  PIC16InstName[i_RTS].name := 'RTS';  //Return from Subroutine
  PIC16InstName[i_RTS].AddAddressMode(aImplied_,$60,1,6,'');
  ...
  PIC16InstName[i_SEC].AddAddressMode(aImplied_,$38,1,2,'');
  PIC16InstName[i_SED].name := 'SED';  //Set Decimal Flag
  PIC16InstName[i_SED].AddAddressMode(aImplied_,$F8,1,2,'');
  PIC16InstName[i_SEI].name := 'SEI';  //Set Interrupt Disable Status
  PIC16InstName[i_SEI].AddAddressMode(aImplied_,$78,1,2,'');
  ...
  PIC16InstName[i_TAX].AddAddressMode(aImplied_,$AA,1,2,'');
  PIC16InstName[i_TAY].name := 'TAY';  //Transfer Accumulator to IndexY
  PIC16InstName[i_TAY].AddAddressMode(aImplied_,$A8,1,2,'');
  PIC16InstName[i_TSX].name := 'TSX';  //Transfer Stack Pointer toIndex X
  PIC16InstName[i_TSX].AddAddressMode(aImplied_,$BA,1,2,'');
  PIC16InstName[i_TXA].name := 'TXA';  //Transfer Index X to Accumulator
  PIC16InstName[i_TXA].AddAddressMode(aImplied_,$8A,1,2,'');
  PIC16InstName[i_TXS].name := 'TXS';  //Transfer Index X to StackRegister
  PIC16InstName[i_TXS].AddAddressMode(aImplied_,$9A,1,2,'');
  PIC16InstName[i_TYA].name := 'TYA';  //Transfer Index Y to Accumulator
  PIC16InstName[i_TYA].AddAddressMode(aImplied_,$98,1,2,'');
